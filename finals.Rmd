...
estimates_table <- read_excel(
  "data/WPP2024_GEN_F01_DEMOGRAPHIC_INDICATORS_COMPACT.xlsx",
  sheet = "Estimates",
  skip = 16,
  col_types = "text"
)
medium_variant_table <- read_excel(
  "data/WPP2024_GEN_F01_DEMOGRAPHIC_INDICATORS_COMPACT.xlsx",
...
medium_clean |>
  ...
  ggplot(aes(x = year, y = total_population_as_of_1_january_thousands)) +
  geom_line(group = 1, color = "blue") +
  geom_point(color = "blue") +
  labs(
    title = "Total Population - World", x = "Year", y = "Population (Billions)"
  ) +
...
medium_clean |>
  filter(
    region_subregion_country_or_area_ %in% c(
      "World",
      "Africa",
      "Asia",
      "Europe",
  ...
  geom_line(group = 1, color = "blue") +
  geom_point(color = "blue") +
  labs(title = "Total Population - World", x = "Year", y = "Population (Billions)") +
  scale_x_continuous(breaks = c(2024, 2050, 2080)) +
  facet_wrap(~
    region_subregion_country_or_area_, scales = "free_y")
```
...
complete_data |>
  ...
  ggplot(
    aes(x = as.numeric(year), y = total_fertility_rate_live_births_per_woman, color = region_subregion_country_or_area_)
  ) +
...
graph_4_data <- complete_data %>%
  ...
  mutate(
    ...
    population_billions = total_population_as_of_1_july_thousands / 1e6
  ) # Convert thousands to billions)

# Create the line graph
ggplot(
  graph_4_data,
  aes(x = year, y = population_billions, color = region_subregion_country_or_area_)
) +
  geom_line(size = 1.2) + # Add lines with increased thickness
  geom_point(size = 2) + # Add larger points for better visibility
  scale_color_manual(values = c(
    "India" = "red", "China" = "blue"
  )) + # Custom colors for India and China
  labs(
...
estimates_clean |>
  ...
  ggplot(
    aes(x = as.numeric(year), y = life_expectancy_at_birth_both_sexes_years, color = region_subregion_country_or_area_)
  ) +
  geom_point() +
  labs(x = "Year", y = "Life Expectancy (Years)", color = "Region") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
...
estimates_clean |>
  ...
  labs(title = "Annual net migration in Ukraine, 1950 to 2023", x = "Year", y = "Net Migration") +
  scale_y_continuous(
    breaks = c(0, -2000, -4000),
    labels = c("0", "-2 Million", "-4 million")
  )
```
...
estimates_clean |>
  filter(year == 2022 &
    region_subregion_country_or_area_ == "World") |>
  summarise(
  ...
  labs(title = "Male-to-Female Population Ratio in the World (2022)", fill = "Gender") +
...
estimates_clean |>
  ...
  filter(
    region_subregion_country_or_area_ %in% c("World", "Asia", "United States of America", "Europe")
  ) |>
  ggplot(
    aes(x = as.numeric(year), y = female_population_as_of_1_july_thousands, color = region_subregion_country_or_area_)
  ) +
  geom_line() +
  geom_point() +
  labs(
...
estimates_clean |>
  ...
  mutate(male_to_female_ratio = male_population_as_of_1_july_thousands / female_population_as_of_1_july_thousands) |>
...
combined_data %>%
  ...
  ggplot(
    aes(x = as.numeric(year), y = region_subregion_country_or_area_, fill = total_fertility_rate_live_births_per_woman)
  ) +
  geom_tile(color = "white") +
  scale_fill_viridis_c(option = "magma", name = "Fertility Rate") +
  labs(title = "Fertility Rate Heatmap (1950-2100)", x = "Year", y = "Country") +
  theme_minimal() +
...
medium_clean %>%
  ...
  ggplot(
    aes(x = region_subregion_country_or_area_, y = avg_fertility_rate, fill = region_subregion_country_or_area_)
  ) +
  geom_bar(stat = "identity", width = 0.6) +
  labs(
    title = "Fertility Rates in Major Continents in 2100",
    x = "Continents",
...
estimates_clean %>%
  ...
  geom_line(size = 1) +
  labs(
    title = "Trends in Fertility Rates Over Time: United States vs. India",
    x = "Year",
    y = "Fertility Rate (Live Births per Woman)",
    color = "Country"
  ) +
...
medium_clean |>
  filter(region_subregion_country_or_area_ == "World") |>
  mutate(year = as.numeric(year)) |>
  ggplot(aes(x = year, y = crude_death_rate_deaths_per_1000_population)) +
  geom_line(group = 1, color = "blue") +
  geom_point(color = "blue") +
  labs(title = "Death Rate - World", x = "Year", y = "Deaths per 1000 People") +
  scale_x_continuous(breaks = c(2024, 2030, 2040, 2050, 2060, 2070, 2080, 2090, 2100)) +
  theme_minimal()
```
...
combined_data |>
  ...
  ggplot(
    aes(x = as.numeric(year), y = crude_death_rate_deaths_per_1000_population, color = region_subregion_country_or_area_)
  ) +
  geom_line(size = .9) +
  geom_point(size = .3) +
  labs(
...
estimates_clean %>%
  ...
  ggplot(
    aes(x = region_subregion_country_or_area_, y = avg_death_rate, fill = region_subregion_country_or_area_)
  ) +
  geom_bar(stat = "identity", width = 0.6) +
  labs(
    title = "Death Rates in Major Continents in 2023",
    x = NULL,
    y = "Deaths per 1000 People",
    fill = "Continent"
  ) +
...
ggplot(
  ...
  aes(x = year, y = life_expectancy, color = region_subregion_country_or_area_)
) +
...
life_expectancy_model <- lm(life_expectancy ~ year + region_subregion_country_or_area_,
  data = train_set
)
...
model_performance <- data.frame(
  RMSE = RMSE(
    test_set$predicted_life_expectancy,
    test_set$life_expectancy
  ),
  R2 = R2(
    test_set$predicted_life_expectancy,
    test_set$life_expectancy
...
ggplot(
  test_set,
  aes(x = life_expectancy, y = predicted_life_expectancy, color = region_subregion_country_or_area_)
) +
...
The scatterplot compares predicted life expectancy at age 65 against actual life expectancy for Afghanistan, Iran, and Vietnam, with the diagonal line representing perfect agreement between the two. Afghanistan’s points are mostly below the diagonal, indicating that the model tends to overestimate life expectancy for the country, likely reflecting the challenges of modeling outcomes in a region heavily affected by conflict and instability. Iran’s points cluster closer to the diagonal, suggesting that the predictions are generally accurate, reflecting more stable socioeconomic conditions and health infrastructure. Vietnam’s points are consistently aligned with or slightly above the diagonal, highlighting the model’s strong predictive performance for the country, likely due to the effectiveness of economic and health reforms that align closely with measurable outcomes. Overall, the scatterplot illustrates varying levels of predictive accuracy across the three countries, reflecting differences in the stability and predictability of factors influencing life expectancy.
...
ggplot(selected_countries_data, aes(x = year)) +
  geom_line(aes(y = life_expectancy, color = region_subregion_country_or_area_),
    size = 1.2
  ) +
  geom_line(
    aes(y = predicted_life_expectancy, color = region_subregion_country_or_area_),
    linetype = "dashed"
  ) +
  labs(
...
The graph compares predicted life expectancy at age 65 (from a linear regression model) with UN-recorded life expectancy for Afghanistan, Iran, and Vietnam over time. Afghanistan consistently has the lowest life expectancy, with gradual improvement and notable declines in the late 1980s and early 1990s. Iran shows steady growth, maintaining a middle ground between the other two countries, with less volatility than Afghanistan. Vietnam demonstrates the highest life expectancy throughout, with significant increases over time and noticeable alignment between predicted and recorded values, particularly in recent years.

The differences in life expectancy trends among Afghanistan, Iran, and Vietnam can be attributed to a variety of historical, political, economic, and healthcare factors unique to each country. Afghanistan's consistently low life expectancy reflects the impact of prolonged conflict, including wars and political instability, which have severely disrupted healthcare systems, infrastructure, and overall living conditions. The declines observed in the late 1980s and early 1990s likely correspond to the aftermath of the Soviet-Afghan War and the subsequent civil unrest, further exacerbating challenges like poverty and limited access to clean water, nutrition, and medical care. In contrast, Iran shows steady growth in life expectancy, likely benefiting from investments in healthcare infrastructure, economic development, and social programs following the 1979 revolution. Despite political challenges and international sanctions, Iran has maintained significant improvements in health outcomes, which may explain its stable upward trend. Vietnam demonstrates the highest life expectancy throughout the period, reflecting the country’s economic reforms (Doi Moi) beginning in the late 1980s, which enhanced healthcare accessibility, reduced poverty, and improved living standards. The alignment of predicted and recorded life expectancy values in recent years for Vietnam suggests the effectiveness of policies aimed at sustaining health and social welfare.

**6.  Requirement-5 (1 pt)**

**Conclusion**

This analysis examines global trends in life expectancy, fertility, death rates, and gender ratios, emphasizing the impact of historical events, socio-economic progress, and demographic shifts. Life expectancy data from Bangladesh, China, and Haiti highlight the devastating effects of natural disasters like the Bhola Cyclone, the Great Chinese Famine, and the Haiti Earthquake, while showcasing long-term resilience and recovery. Trends in life expectancy at age 65 in Afghanistan, Iran, and Viet Nam reveal the influence of conflicts, such as the Soviet-Afghan War and the Vietnam War, alongside post-conflict recovery and socio-economic reforms like Vietnam's Doi Moi. Death rate analyses underscore Europe's aging population as a driver of its high 2023 death rate and illustrate global convergence of death rates over time, with notable spikes during crises like COVID-19. Fertility data reveal declining rates in the US and India, attributed to modernization and urbanization, and project Africa's stabilization near replacement levels by 2100, in contrast to declines in Asia and Europe due to advanced development. Heatmap insights highlight country-specific fertility transitions, with Nigeria and Saudi Arabia experiencing sharp declines amid economic growth, while Japan and China exhibit consistently low fertility rates. Gender ratio analyses reveal Asia's male-dominant population, shaped by policies like the One Child Policy, contrasted with Europe’s surplus of females due to longer life expectancy. Together, these findings underscore the interplay of historical, economic, and cultural factors shaping global demographic trends, offering insights into resilience, progress, and evolving population dynamics.
