...
```
The shape of the line graphs suggests that male and females in their respective countries are expected to grow at a similiar rate; however, a side by side comparison suggests otherwise
Graph 3
```{r}
estimates_clean |>
...
```
GRAPH 5
Heatmap: Fertility rate for multiple countries over decades
# Create a heat map 
```{r}
...
```
GRAPH 6
Bar chart:  Fertility rates in major continents of the world in year 2100
# Filters data for the year 2100 and the major continents 
...
GRAPH 7
Line chart: Trends in Fertility Rates Over Time: United States vs. India
# Filter data for US and India and create the line chart
```{r}
estimates_clean %>%
